...
int leer_csv(const char *nombre_archivo, order *orders, int max_ventas)
{
    ...
    while (fgets(linea, sizeof(linea), archivo)&& total_ventas < max_ventas)
    { // Lee una línea del archivo CSV
        ...
        while (*ptr) { // Mientras no lleguemos al final de la línea (carácter nulo '\0')
            ...
            { // Si encontramos una coma fuera de comillas
        ...
        orders[total_ventas++] = v; // Guardamos la estructura en el arreglo y aumentamos el contador
