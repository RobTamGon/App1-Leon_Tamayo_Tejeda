...
char* encontrar_promedio_pizzas_orden(int *largo, Venta *ventas) {
    ...
    int *ordenes = malloc((*largo) * sizeof(int)); // Reservamos espacio de sobra
    if (!ordenes) {
    ...
    for (int i = 0; i < *largo; i++) {
        ...
        if (!ya_contada) {
            ordenes[cantidad_ordenes++] = ventas[i].order_id; // Añadimos el órden a la lista
        }

        suma_pizzas += ventas[i].quantity; // Suma de pizzas por orden
    }
...
char* encontrar_promedio_pizzas_por_dia(int *largo, Venta *ventas) {
    ...
    char **dias = malloc((*largo) * sizeof(int)); // Reservamos espacio de sobra
    if (!dias) {
        perror("Error al asignar memoria."); // Error al asignar memoria
        return NULL;
    }
    ...
    for (int i = 0; i < *largo; i++) {
        ...
        if (!ya_contado) {
            dias[cantidad_dias] = malloc(strlen(ventas[i].order_date) + 1); // Reservamos espacio
            ...
            strcpy(dias[cantidad_dias], ventas[i].order_date); // Copiamos el día
            cantidad_dias++; // Aumentamos el contador de la cantidad de días
        }

        suma_pizzas += ventas[i].quantity; // Suma de pizzas por día
    }
...
char* encontrar_categorias_vendidas(int *largo, Venta *ventas) {
    Categoria *categorias = malloc((*largo) * sizeof(Categoria)); // Reservamos memoria para categorías
    if (!categorias) {
        perror("Error al asignar memoria."); // Error al asignar memoria
        return NULL;
    }
    ...
    for (int i = 0; i < *largo; i++) {
        ...
        for (int j = 0; j < total_categorias; j++) {
            if (strcmp(categorias[j].nombre, ventas[i].pizza_category) == 0) { // Comprobamos si la categoría coincide
        ...
        if (idx == -1) {
            strcpy(categorias[total_categorias].nombre, ventas[i].pizza_category); // Copiamos la categoría
        ...
        categorias[idx].cantidad += ventas[i].quantity; // Sumamos la cantidad de pizzas
    }
