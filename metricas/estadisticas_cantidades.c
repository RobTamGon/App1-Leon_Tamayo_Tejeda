...
char* ventas_categoria(int *size, order *orders) {
	...
	for (int i = 0; i < *size; i++) {
		...
		for (int j = 0; j < total_categorias; j++) {
			if (strcmp(categorias[j].nombre, orders[i].pizza_category) == 0) { // Comprueba si la categoría coincide
	...
	for (int i = 0; i < total_categorias; i++) {
		if (categorias[i].cantidad > 0) {
			if (!primera_categoria) { // Añade una coma antes de añadir la siguiente categoría
				strcat(respuesta, ", ");
			}
